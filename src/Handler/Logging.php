...
use Error;
use ErrorException;
use ErrorHandlerCustom\ErrorHanlderCustomConstant;
use Exception;
...
class Logging
{
     ...
     * @param array                     $errorHandlerCustomLocalConfig
    ...
    public function __construct(
        ...
        $requestUri = '',
        array              $errorHandlerCustomLocalConfig,
        ...
        $this->configLoggingSettings = $errorHandlerCustomLocalConfig['logging-settings'];
        $this->logWritersConfig      = $logWritersConfig;
        $this->mailMessageService    = $mailMessageService;
        $this->mailMessageTransport  = $mailMessageTransport;
        $this->emailReceivers        = $errorHandlerCustomLocalConfig['email-notification-settings']['email-to-send'];
        $this->emailSender           = $errorHandlerCustomLocalConfig['email-notification-settings']['email-from'];
    }
    ...
    private function collectErrorExceptionData($e)
    {
        if ($e instanceof ErrorException && isset(Logger::$errorPriorityMap[$severity = $e->getSeverity()])) {
            ...
            $errorType = ErrorHanlderCustomConstant::ERROR_TYPE[$severity];
        } else {
            $priority  = Logger::ERR;
